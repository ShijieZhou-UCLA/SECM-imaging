...
lambda_list = [0.005, 0.01, 0.03, 0.04];

n_shift  = length(shift_list);
n_lambda = length(lambda_list);
loss_list = zeros(n_shift, n_lambda);

for i_shift = 1:n_shift
    shift  = shift_list(i_shift);
    fprintf('\nShift %d\n', shift);
    ...
    for i_lambda = 1:n_lambda
        ...
        psf_params = [.6, .1, 5, 2, 1.5, 8, shift]';
        ...
        lda = lambda * max(pos(D*back_project(lines))); % 0.05*
        % lda = 0;
        ...
        img = alg.vars{1} * D;
        img.normalize();
        loss_list(i_shift,i_lambda) = img.min_square_loss(x_truth,D);
        
        fprintf('%.2f\n', loss_list(i_shift,i_lambda));
        
        close all
        
        save('../loss_save_2.mat', 'loss_list', 'shift_list', 'lambda_list')
